...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("CadornaCoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("CadornaCoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "CadornaCoin:");
    }
...
{
    QString ret = QString("CadornaCoin:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "CadornaCoin.desktop";
    return GetAutostartDir() / strprintf("CadornaCoin-%s.lnk", chain);
}
