...
- [Building Litecoin Core](#building-CadornaCoin-core)
...
Clone the git repositories for CadornaCoin and Gitian.
...
in the CadornaCoin repository under 'Fetch and create inputs' to install sources which require
...
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the CadornaCoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/CadornaCoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/CadornaCoin-project/CadornaCoin
...
For example:
```bash
URL=https://github.com/thrasher-/CadornaCoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit CadornaCoin=${COMMIT} --url CadornaCoin=${URL} ../CadornaCoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit CadornaCoin=${COMMIT} --url CadornaCoin=${URL} ../CadornaCoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit CadornaCoin=${COMMIT} --url CadornaCoin=${URL} ../CadornaCoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the CadornaCoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../CadornaCoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/CadornaCoin
SIGPATH=/some/root/path/CadornaCoin-detached-sigs

./bin/gbuild --url CadornaCoin=${BTCPATH},signature=${SIGPATH} ../CadornaCoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/CadornaCoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/CadornaCoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/CadornaCoin-osx-build.assert
```
...
[CadornaCoin-project/gitian.sigs.ltc](https://github.com/CadornaCoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
