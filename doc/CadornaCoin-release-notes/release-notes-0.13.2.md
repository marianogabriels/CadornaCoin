Litecoin Core version 0.13.2 is now available from:
  ...
  <https://download.CadornaCoin.org/CadornaCoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/CadornaCoin-project/CadornaCoin/issues>
...
updates will be released anymore. Without security updates, using a CadornaCoin
...
This allows running CadornaCoind without having to do any manual configuration.
...
`CadornaCoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /CadornaCoinrpc>
        ...
        # optional bypass CadornaCoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< CadornaCoinrpc:<password>
    </Location>
...
Restarting CadornaCoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in CadornaCoin.conf
...
CadornaCoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/CadornaCoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `CadornaCoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `CadornaCoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of CadornaCoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of CadornaCoins being spent is exactly
...
- `CadornaCoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `CadornaCoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
